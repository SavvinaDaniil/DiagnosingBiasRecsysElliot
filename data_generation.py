...
def popularity_to_rating(popularities, rating_range):
    mean_ratings = np.round((popularities-np.min(popularities))/(np.max(popularities)-np.min(popularities))*(rating_range[1]-1)+rating_range[0])
    return mean_ratings

def generate_data(strategy, rating_range = [1,10], random_seed = 0, user_col = "user", item_col = "item", predict_col = "rating", 
                       copying_dataset = None, average_rating = None, user_perc=None):
    ...
    if strategy == "uniformly_random":
    ...
    elif strategy == "personal_normal": # users rate random but each with their own distribution (around their own average)
        ...
        indices_users = np.random.choice(num_users, int(user_perc*num_users), replace=False, )
        
        i=0
        for item in item_dist.index.values:
            mu = mean_ratings[i]
            length = len(ratings[(ratings.item==item)&(ratings.user.isin(indices_users))])
            rates = truncated_poisson(mu, rating_range, length)
            ratings.loc[(ratings.item==item)&(ratings.user.isin(indices_users)), "rating"] = rates
            i+=1
