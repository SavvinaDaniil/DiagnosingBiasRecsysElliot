...
def user_rating_distribution(
    df_events, user_col, item_col, predict_col="rating", verbose=False
):
    user_dist = (
        df_events.groupby(user_col).mean()[predict_col].sort_values(ascending=False)
    )
    # num_users = len(user_dist)
    if verbose:
        print("Mean " + item_col + "s per user: " + str(np.round(user_dist.mean(), 1)))
...
def item_rating(df_events, item_col, predict_col, verbose=False):
    ...
    if verbose:
        print(
            ...
            + item_col
            + ": "
            + str(np.round(item_rating[predict_col].mean(), 1))
        )
        print(
            "Min rating per "
            + item_col
            + ": "
            + str(np.round(item_rating[predict_col].min(), 1))
        )
        print(
            "Max rating per "
            + item_col
            + ": "
            + str(np.round(item_rating[predict_col].max(), 1))
        )
        print("\n")
    return item_rating
...
def plot_data_distribution(
    item_dist, user_dist, item_ratings, user_ratings, item_col, user_col
):
    font = {"size": 20}
...
def plot_rating_distribution(
    all_rating_values, user_rating_values, item_rating_values, rating_range
):
    font = {"size": 20}
    matplotlib.rc("font", **font)
    ...
    n, bins, patches = ax1.hist(
        xa,
        bins=np.arange(rating_range[1] + 2) - 0.5,
        facecolor="#2ab0ff",
        edgecolor="#e0e0e0",
        linewidth=0.7,
        alpha=0.8,
    ...
    ax1.axvline(
        xa.mean(), color="k", linestyle="dashed", linewidth=1, label="Mean rating"
    )
    ...
    # ax1.title('Rating distribution', fontsize=20)
    ax3.axvline(
        xi.mean(), color="k", linestyle="dashed", linewidth=1, label="Mean rating"
    )
...
def plot_popularity_distribution(pop_fraq, item_col, dividing=[False, 0]):
    ...
    plt.axhline(
        ...
        color="black",
        linestyle="--",
        label="80% ratio of popular " + item_col + "s",
    )
...
def calculate_popularity(
    df_events, top_item_dist, item_dist, num_users, user_col, item_col
):
...
def sort_user_dist(
    user_dist, pop_count, user_hist, pop_fraq, pop_item_fraq, by="pop_fraq"
):
...
def split(user_dist_sorted, top_fraction):
    low, med, high = np.split(
        user_dist_sorted,
        [
            int(top_fraction * len(user_dist_sorted)),
            int((1 - top_fraction) * len(user_dist_sorted)),
        ],
    )
    return low, med, high
