...
def calculate_userKNN_characteristics(
    ratings,
    user_dist,
    item_dist,
    user_col,
    ...
    pop_count, user_hist, pop_fraq, pop_item_fraq = analysis.calculate_popularity(
        ratings, top_item_dist, item_dist, num_users, user_col, item_col
    )
    # merge the calculated characteristics in the user_dist
    all_users = analysis.sort_user_dist(
        user_dist, pop_count, user_hist, pop_fraq, pop_item_fraq, by=divide_by
    )
    # calculate average rating per user
    df_user_rating = ratings[[user_col, predict_col]].groupby(user_col).mean()
    # merge rating and other characteristics
    user_characteristics = pd.merge(
        all_users, df_user_rating, left_index=True, right_index=True
    ...
    item_characteristics = pd.merge(
        df_item_rating, df_item_dist, left_index=True, right_index=True
    )
    # average global ratings of every user's consumed items
    user_characteristics["global_rating"] = global_rating(
        user_characteristics.index, user_items_rated, item_characteristics.rating
    )
    ...
    user_characteristics["score_vs_pop"] = score_vs_pop(
        user_characteristics.index, ratings, item_characteristics
    )
...
def st_dev(ratings, user_col, predict_col):
    user_variance = (
        ratings.groupby(user_col).std().rename(columns={predict_col: "st_dev"})
    )
    ...
    for index in users:
        ...
        if neighbours > 0:
            non_zero_similarities = (
                user_similarities.loc[index][user_similarities.loc[index] != 0]
                .drop(index)
                .values
            )
...
def calculate_user_similarities(ratings_pivot, mean_centered):
    ...
    if mean_centered:
        ratings_pivot["mean"] = ratings_pivot.replace(0, np.NaN).mean(
            axis=1, skipna=True
        )
        ratings_pivot = (
            ratings_pivot.replace(0, np.NaN)
            .sub(ratings_pivot["mean"], axis=0)
            .fillna(0.0)
            .drop("mean", axis=1)
        )

    user_similarities = pd.DataFrame(
        sklearn_cs(ratings_pivot),
        index=ratings_pivot.index,
        columns=ratings_pivot.index,
    )
...
def find_common_items(user_1, user_2):
    user_1_new = [
        user_1[i] for i in range(len(user_1)) if (user_1[i] != 0) & (user_2[i] != 0)
    ]
    user_2_new = [
        user_2[i] for i in range(len(user_2)) if (user_1[i] != 0) & (user_2[i] != 0)
    ]
...
def calculate_sparsity_cumulatively(ratings, df_item_dist, num_users_total):
    ...
    for item_set in item_sets:
        ...
        sparsity = (num_users_total * num_items - len(item_ratings)) / (
            num_items * num_users_total
        )
...
def plot_relation(
    df, characteristic_x, characteristic_y, name_x, name_y, extra_title=""
):
