...
def calculate_ACLT(train_df, recs, user_col, item_col, limit = 0.2):
    ...
    print("nr of longtail", len(recs[recs[item_col].isin(longtail_items_in_training_set)]))
...
def plot_results(dfs, GAP, algo_name, loss,precision, recall,ndcg, stdev_20, ACLT, cv, n, args, data_strategy, save_plot=False):
    font = {'size'   : 15}
    ...
    if save_plot:
        plt.savefig('graphs/'+algo_name+'/'+data_strategy+'_'+str(args)+'.png')
    plt.show()
...
def train_algorithm_cornac(algorithm, algo_name, ratings, evaluation_way, partition_way, sampling_strategy = None,  verbose = True, min_sim = 0, user_col="user", item_col="item", predict_col = "rating", n = 10, plot = False, args = {}, data_strategy = "", save_plot = False):
    ...
    if evaluation_way == "simple_split":
        ...
        models = [algorithm]
        metrics = [RMSE()]
        exp = cornac.Experiment(eval_method=eval_method, models=models, metrics=metrics, user_based=False,save_dir="cornacLogs", verbose=verbose)
        exp.run()
        ...
        ARP, ave_PL, ACLT = calculate_all_pb_metrics(pop_bias, test_users, item_col, user_col, train_df, recs_grouped, recs)
        pop_corr = calculate_pop_correlation(pop_bias)
        metrics_dict = {"pop_corr":pop_corr, "ARP":ARP, "ave_PL": ave_PL, "ACLT": ACLT, "AggDiv": AggDiv, "RMSE":loss, 'NDCG':ndcg}
        #print(metrics_dict)
        #print(ARP, ave_PL, ACLT, ACST, AggDiv)
        if plot:
            plot_results(pop_biases.copy(), GAP_vs_GAP.copy(), algo_name, loss,precision, recall,ndcg, stdev_20, ACLT,cv=False, n=n, args=args, data_strategy=data_strategy, save_plot=save_plot)
        ...
        metrics_dict = {"pop_corr":pop_corr, "RMSE":total_loss,'NDCG':total_ndcg, "ARP":total_ARP, "ave_PL": total_ave_PL, "ACLT": total_ACLT, "AggDiv": AggDiv}
        #print(metrics_dict)
        if plot:
            plot_results(pop_biases.copy(), total_GAP_vs_GAP.copy(), algo_name, total_loss,total_precision,total_recall, total_ndcg, total_stdev_20, total_ACLT,cv=True, n=n, args=args, data_strategy=data_strategy, save_plot=save_plot)
        
        return pop_biases, metrics_dict
    

def train_algorithm(algorithm, algo_name, ratings, evaluation_way, partition_way, sampling_strategy = None, verbose = True, min_sim = 0, user_col="user", item_col="item", predict_col = "rating", n=10, fallback = False, plot = False, args = {}, data_strategy = "", save_plot = False):
    all_items=set(ratings.item.unique())
    if partition_way == "user":
        if sampling_strategy == "frac":
            sample = xf.SampleFrac(0.2, rng_spec=0)
    ...
    if evaluation_way == "simple_split":
        ...
        metrics_dict = {"pop_corr":pop_corr, "RMSE":loss, 'NDCG':ndcg,"ARP":ARP, "ave_PL": ave_PL, "ACLT": ACLT, "AggDiv": AggDiv}
        #print(metrics_dict)
        #print(ARP, ave_PL, ACLT, ACST, AggDiv)
        if plot:
            plot_results(pop_biases.copy(), GAP_vs_GAP.copy(), algo_name, loss, precision, recall,ndcg,mean_std_first_20,ACLT,cv=False, n=n, args=args, data_strategy=data_strategy, save_plot=save_plot)
        return pop_biases, metrics_dict
        ...
        #print(metrics_dict)
        if plot:
            plot_results(pop_biases.copy(), total_GAP_vs_GAP,algo_name, total_loss,total_precision,total_recall, total_ndcg, total_st_dev_20,total_ACLT, cv=True, n=n, args=args, data_strategy=data_strategy, save_plot=save_plot)
